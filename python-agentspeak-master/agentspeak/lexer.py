...
"""
Repasar el documento y mirar el sys.stdin.isatty()
"""
...
class Token(object):
    def __init__(self, regex,
                 ...
                 unary_op=None, mult_op=None, add_op=None, comp_op=None, hola=None):
        self.re = re.compile(regex)

        self.space = space # Whitespace
        ...
        self.goal_type = goal_type # Goal type
        self.formula_type = formula_type # Formula type

        self.unary_op = unary_op # Unary operator
        self.mult_op = mult_op # Multiplicative operator
        self.add_op = add_op # Additive operator
        self.comp_op = comp_op # Comparison operator
...
def tokenize(sourcefile, log, firstline=1):
    in_comment = False # Are we in a comment?

    for lineno, line in enumerate(sourcefile): # For each line in the file
        line = line.replace("\t", "    ") # Replace tabs with spaces
        startcol = 0 # Start column

        while startcol < len(line): # While we haven't reached the end of the line
            # Skip comments.
            if in_comment: # If we're in a comment
                match = RE_END_COMMENT.match(line, startcol) # Find the end of the comment
                if not match: # If we didn't find the end of the comment
                    break # Break out of the loop
                else: # If we did find the end of the comment
                    in_comment = False # We're no longer in a comment
                    startcol = match.end(0) # Set the start column to the end of the comment
                    continue # Continue to the next iteration of the loop

            match = RE_START_COMMENT.match(line, startcol) # Find the start of a comment
            if match: # If we found the start of a comment
                in_comment = True # We're now in a comment
                startcol = match.end(0) # Set the start column to the end of the comment
                continue

            # Scan next token.
            for token_type in TokenType: # For each token type
                token = token_type.value # Get the token
                match = token.re.match(line, startcol) # Find the token in the line
                if not match: # If we didn't find the token
                    continue

                if not (token.space or token.comment): # If the token isn't a space or a comment
                    # Yield the token. (We found a token!) 
                    # If the token is an hola, print hola and break the execution
                    if token.hola:
                        print("hola")
                        break
                    yield TokenInfo( 
                ...
                startcol = match.end(0) # Set the start column to the end of the token
                break
            else:  
...
class TokenStream(object):
    def __init__(self, source, log, firstline=1):
        self.tokens = tokenize(source, log, firstline=firstline) # Get the tokens
        self.tok = None 

    def peek(self):
        return self.tok # Return the current token

    def __next__(self):
        self.tok = next(self.tokens) # Get the next token
        return self.tok # Return the current token
...
def main(source, lineno=1):
    log = agentspeak.Log(agentspeak.get_logger(__name__), 3) # Get the logger

    for tok in tokenize(source, log, lineno): # For each token
        log.info("%s", tok.lexeme, loc=tok.loc) # Print the token

    log.throw() # Throw any errors


def repl():
    lineno = 1 # Line number
    while True: # While we're in the REPL
        try: # Try to get input
            line = agentspeak.util.prompt("agentspeak.lexer >>> ") # Get the input
            main(agentspeak.StringSource("<stdin>", line), lineno) # Run the lexer
            lineno += 1 # Increment the line number
        except agentspeak.AggregatedError as error: # If we got an error
            print(str(error), file=sys.stderr) # Print the error
        except KeyboardInterrupt: # If we got a keyboard interrupt
...
if __name__ == "__main__": 
    try:
        ...
        if args: # If we got arguments
            for arg in args: # For each argument
                with open(arg) as source:  # Open the file
                    main(source) # Run the lexer
        elif sys.stdin.isatty(): # If we're in the REPL
            repl() # Run the REPL
        else: # If we're not in the REPL
            main(sys.stdin) # Run the lexer
    except agentspeak.AggregatedError as error: # If we got an error
        print(str(error), file=sys.stderr) # Print the error
